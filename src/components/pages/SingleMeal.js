...
import { StyledSingleMeal, Info, Recipe, Description } from '../styles/SingleMeal.styled'

function SingleMeal() {
  const { singleMeal, loading } = useSelector((state) => state.getMealById)
  const dispatch = useDispatch()
  const { idMeal } = useParams()

  useEffect(() => {
    dispatch(getMealByIdThunk(idMeal))
  }, [])
